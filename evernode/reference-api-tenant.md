...
| Memos  | array of objects | This array contains two objects. The following is an example and the objects are in the same order in the response:<br> <pre lang="json">{<br> type: 'evnAcquireError',<br> format: 'text/json',<br> data: '{"type":"ACQUIRE_ERR","reason":"docker_image_invalid"}'<br>},<br>{<br> type: 'evnAcquireError',<br> format: 'text/json',<br> data: '{"type":"ACQUIRE_ERR","reason":"docker_image_invalid"}'<br>}</pre>      |
