...
1. Let's add following code to request EVRs.
   - You'll be awarded 6000 EVRs to your tenant account for the beta testing
   ```javascript
...
1. So, Now you have a tenant account with enough EVRs. Let's pick a host to acquire an instance.
   ...
   - You first need a instantiate a registry client. 
   - Then call the `connect()` method on registry client to prepare.
   ```javascript
...
1. Now you have purchased an instance for an one moment (Approx 1 hour). Let's extend the instance life for another one moment (Approx 1 hour).
   ```javascript
      if (instanceName) {
         try {
...
2. You've generated the `evernode-starter-client` client now. So, let's go back to our previous `evernode-project` directory to acquire the evernode instance.
   - Now run, following command to acquire the instance. (Replace tge public key with the public key in your previous output.)
   ```
...
1. Now you have an evernode instance which is ready to be deployed a [smart contract](../hotpocket/concepts.md#smart-contract). So, let's deploy the contract to the evernode instance.
2. For the easiness of understanding let's upload the smart contract that you've created before in [hpdevkit basic tutorial](../hotpocket/tutorial-basics.md).
   ...
   - So, here we upload that same simple smart contract to the evernode.
...
3. First, we need to prepare a smart contract bundle which can be uploaded to evernode instance via the [evernode-starter-client](#step-3---pick-an-active-available-host-to-acquire).
   ...
   cd mycontract
   npm install
   # Bundle command will be implemented in the future.
   ```

4. Now the bundle is generated. Lets connect the previously created `evernode-starter-client` to the acquired evernode instance.
